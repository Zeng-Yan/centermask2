# defining funcs that modify the graph of onnx
# provide a replacement of torch.nonzero() (2-D) by TopK in onnx,
# Author: zengyan
# Final: 21.08.24
...
def make_NonZeroByTopK(x: str, top: str, y: str) -> (list, list):
    """
    create onnx nodes and initializers to replace 2D-NonZero() via TopK() due to the bad support of NonZero() in ATC tools of Huawei
    """
    # compute k
    node_sum = helper.make_node('ReduceSum', [x], ['input_sum'], keepdims=0)  # scala里的reduce
    ...
    # compute idx
    constant_1 = helper.make_tensor('c_1', TensorProto.INT64, [1], [1])  # [1]
...
if __name__ == '__main__':
    ...
    lst_nodes, lst_init = make_NonZeroByTopK('x', 'top', 'y')  # make nodes and initializer
    ...
    output = torch.nonzero(input_tensor)
    array = output.detach().cpu().numpy()
    print(f'\nNonzero() of input: \n{array}\nshape: \n{output.shape}')
    input_tensor = input_tensor.detach().cpu().numpy()
    limit = torch.tensor([0], dtype=torch.int64).detach().cpu().numpy()
    onnx_session = onnxruntime.InferenceSession('NonzeroByTopK.onnx')
    output = onnx_session.run(['y'], {'x': input_tensor, 'top': limit})
